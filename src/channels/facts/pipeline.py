...
class FactsPipeline(ContentPipeline):
    """Pipeline for educational facts content."""
    ...
    def __init__(
        ...
        self.config: ChannelConfig = CHANNEL_CONFIGS[ChannelType.FACTS]
    ...
    async def run(self, channel: ChannelType) -> VideoProject:
        ...
        project = VideoProject(
            id=project_id,
            channel=ChannelType.FACTS,
            script=script,
            output_path=output_path,
        )
        logger.info("facts_pipeline_start project_id=%s", project_id)
        ...
        try:
            ...
            logger.info("facts_pipeline_complete project_id=%s", project_id)
        except PipelineError:
            raise
        except Exception as e:
            logger.error("facts_pipeline_failed project_id=%s error=%s", project_id, e)
            project.mark_failed(error=str(e))
            raise PipelineError(f"Facts pipeline failed: {e}") from e
    ...
    async def run_batch(self, channel: ChannelType, count: int) -> AsyncIterator[VideoProject]:
        ...
        for topic in topics:
            ...
            try:
                ...
                logger.error("facts_batch_item_failed project_id=%s error=%s", project_id, e)
                project.mark_failed(error=str(e))
            yield project
    ...
    async def _build_script(self, topic: dict[str, Any]) -> Script:
        duration = self._cfg("target_duration_minutes", 7)
        ...
        return Script(
            ...
            body=body,
            cta="Subscribe for more mind-blowing facts!",
            channel=ChannelType.FACTS,
        )
    ...
    def _validate_script(self, script: Script) -> None:
        ...
        if wc < 800:
            raise PipelineError(f"Script too short: {wc} words (min: 800)")
        if wc > 1500:
            raise PipelineError(f"Script too long: {wc} words (max: 1500)")
    ...
    async def _generate_visuals(self, output_path: Path, script: Script) -> list[Path]:
        ...
        for i, scene in enumerate(scenes):
            prompt = VISUAL_PROMPT_TEMPLATE.format(
                ...
                mood=scene.get("mood", "educational, bright"),
    ...
    async def _create_video_clip(self, image_path: Path, output_path: Path, duration: int) -> None:
        motion_prompt = "slow zoom with subtle educational movement"
    ...
    def _extract_scenes(self, script: Script) -> list[dict[str, Any]]:
        ...
        est_dur = (script.word_count / 150) * 60  # 150 WPM for educational
    ...
    async def _upload_video(
        ...
        logger.info("facts_video_uploaded project_id=%s", project.id)
